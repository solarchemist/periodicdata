periodic_plot_discrete <- function(pdata = periodicdata, ptitle = "Periodic Table of the Elements", psubtitle = "Created with the periodicdata package", plegendtitle = "", plegendcols = 3) {
...
p <- ggplot() +
   ...
   scale_colour_discrete(
      na.value = "grey70",
      # don't display NA factor in legend
      # https://stackoverflow.com/questions/45493163/ggplot-remove-na-factor-level-in-legend
      na.translate = FALSE
   ) +
   # size of legend boxes independent of geom_point's aes
   guides(colour = guide_legend(
      override.aes = list(size = 5),
      ncol = plegendcols, byrow = TRUE, title.hjust = 0)) +
   ### LABELS
   ...
   theme(
      ...
      # white background means NA elements become invisible
