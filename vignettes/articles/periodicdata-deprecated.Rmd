...
# https://pkgs.rstudio.com/rmarkdown/reference/html_vignette.html
bibliography: ../references.bib
---


> This document used to be a vignette but should be considered deprecated.
> /TA 2025-05-10
...
library(ggrepel)
library(scales)
library(svglite) # ggsave to svg format
library(here)
```
...
ggplot(p.data, aes(y = 10^Production, x = as.numeric(Atomic_Number))) +
   geom_line(colour = "gray", linewidth = 0.75) +
   geom_point() +
...
## Split quantities into separate dataframes for values and unit

We now have several tasks ahead of us before the data is usable:

+ what type of data each property is (character string, dimensionless quantity, or quantity),
+ determine and identify what part of each property value is the value and which is the unit,
+ extract value and unit separately from each datapoint and save to `values` and `units` dataframe,
  respectively.

Take care that the dataframes `values` and `units` have the same dimensions
(rows/columns). The point behind splitting each quantity up is to be able to treat
the value numerically, plot it, etc. It's therefore crucial that the dataframes
stay the same size, so that once a value is read from `values`, we can trust that
...
## Re-join cleaned up values and units

Because I would like to convert the complete dataset to YAML.
```{r, echo=TRUE}
# how to merge two dataframes with the same dimensions
# I want paste() for each cell, effectively
# https://stackoverflow.com/questions/70955329/merge-two-data-frames-cell-wise
properties_clean <-
   data.frame(matrix(
      data = "",
      nrow = dim(values)[1],
      ncol = dim(values)[2],
      dimnames = list(seq(1, dim(values)[1]), names(values)),
      byrow = TRUE))
for (i in 1:dim(values)[1]) {
   for (j in 1:dim(values)[2]) {
      properties_clean[i,j] <-
         paste(
            ifelse(is.na(values[i,j]), "", values[i,j]),
            ifelse(is.na(units[i,j]), "", units[i,j])
         ) %>% trimws()
   }
}
```

Now convert this dataframe to YAML.
```{r, echo=TRUE, eval=FALSE}
# What is the problem here on package rebuild!? Does not make sense...
# Right, vignettes are not allowed to read files in data-raw.
# Put stuff for vignettes in inst/extdata or something instead.
#  <error/rlang_error>
#   Error in `file()`:
...
#       ▆
#    1. └─yaml::write_yaml(...)
#    2.   └─base::file(file, "w", encoding = fileEncoding) at yaml/R/write_yaml.R:8:9
write_yaml(x = properties_clean, column.major = TRUE, file = here::here("data-raw", "periodicdata-colmajor.yml"))
write_yaml(x = properties_clean, column.major = FALSE, file = here::here("data-raw", "periodicdata-colminor.yml"))
```
...
# create.dir=T should create directory even if it does not already exist
ggsave(
   filename = here::here("doc", "periodictable-ggplot.png"), create.dir = TRUE,
   plot = p.periodictable_ggplot2, width = ptable.width, height = ptable.height)
ggsave(
   filename = here::here("doc", "periodictable-ggplot.pdf"), create.dir = TRUE,
   plot = p.periodictable_ggplot2, width = ptable.width, height = ptable.height)
ggsave(
   filename = here("doc", "periodictable-ggplot.svg"), create.dir = TRUE,
   plot = p.periodictable_ggplot2, width = ptable.width, height = ptable.height)
```
